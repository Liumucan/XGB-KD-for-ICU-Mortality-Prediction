...
def add_age_to_icustays(stays):
    admittime_years = (pd.to_datetime(stays.INTIME).dt.year * 365 +
                       pd.to_datetime(stays.INTIME).dt.month * 30 +
                       pd.to_datetime(stays.INTIME).dt.day) / 365
    dob_years = (pd.to_datetime(stays.DOB).dt.year * 365 +
                 pd.to_datetime(stays.DOB).dt.month * 30 +
                 pd.to_datetime(stays.DOB).dt.day) / 365
    stays['AGE'] = admittime_years - dob_years
